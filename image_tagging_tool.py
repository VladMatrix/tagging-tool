...
class ImageTaggingTool:
	...
	def _load_current_pair(self):
		...
		self.scale_to(self.canvas0, 0)
		self.scale_to(self.canvas1, 0)
	...
	def redraw_points(self, canvas: Canvas):
		...
		for i, point in enumerate(canvas.points):
			if i == canvas.selected_tag_idx:
				outline = "#FFFFFF"
				pt_size = PT_BASE_SIZE + PT_SELECTED_EXTRA_SIZE + PT_ZOOM_SCALE_FACTOR * (canvas.scale_idx + 1)
			else:
				outline = "#000000"
	...
	def pan_image(self, event):
		if not self.tag_mode:
			...
			print(event.widget.coords("image"))
	...
	def scale_up(self, event):
		...
		if canvas.scale_idx + 1 < len(IMG_SCALES):
