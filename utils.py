...
class Canvas(tk.Canvas):
    def __init__(self, *args, **kwargs):
        ...
        self.zoom_scale = 1
        self.scale_idx = NEUTRAL_ZOOM_IDX
...
# This is a band-aid solution for a display issue caused by a long data directory path.
# Displaying a long path causes all widgets to shift to the right, screwing up the display
# The issue can theoretically persist if the dirname itself is long enough
def get_display_dir(data_dir: str) -> str:
    path_components = os.path.normpath(data_dir).split(os.path.sep)
    display_dir = str(os.path.sep).join(path_components[-2:])
    if data_dir[0] == os.path.sep:
        display_dir = f"{os.path.sep}{display_dir}"
